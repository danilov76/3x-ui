...
func TestCallTunnelRemoteRoundTrip(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Secret != "test-shared-secret" || req.Role != "server" {
			t.Error("wrong install payload")
		}
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]any{"success": true, "obj": calltunnel.Status{Provider: "vk", Installed: true, Role: "server", State: "active", Fingerprint: "public-fingerprint"}})
	}))
	defer server.Close()
	remote := NewRemote(nodeForServer(t, server, "skip", ""), nil)
	st, err := remote.CallTunnel(context.Background(), "vk", "install", calltunnel.Request{Role: "server", Secret: "test-shared-secret"})
	if err != nil || st.Fingerprint != "public-fingerprint" {
